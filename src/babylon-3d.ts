...
function toColor4(BABYLON: any, css: string): any {
  ...
  if (Number.isNaN(n) || hex.length !== 8) return new BABYLON.Color4(0, 0, 0, 1)
...
const warnDeprecated = (): void => {
  ...
  console.warn(
    `<tosi-3d> is deprecated — use tosijs-3d instead (https://www.npmjs.com/package/tosijs-3d).\n` +
  ...
  connectedCallback(): void {
    ...
    this.babylonReady.then(async (BABYLON) => {
      ...
      this.engine = new BABYLON.Engine(
        canvas,
        true,
        clearColor && clearColor.a < 1
          ? { alpha: true, premultipliedAlpha: false }
          : undefined
      )
      this.scene = new BABYLON.Scene(this.engine)
      if (clearColor) this.scene.clearColor = clearColor
      ...
      if (this.src) {
        ...
        await this.loadScene(
          this.src.slice(0, slash + 1) || '/',
          this.src.slice(slash + 1)
        )
      }
      if (this.scene.activeCamera === undefined) {
