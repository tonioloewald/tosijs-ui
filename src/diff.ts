...
export function diffBlocks(lines: DiffLine[]): DiffBlock[] {
  const blocks: DiffBlock[] = []
  for (const line of lines) {
    const last = blocks[blocks.length - 1]
    if (line.op === 'context') {
      if (last?.kind === 'context') last.lines.push(line.text)
      else blocks.push({ kind: 'context', lines: [line.text] })
      continue
    }
    const block =
      last?.kind === 'change'
        ? last
        : (blocks[
            blocks.push({ kind: 'change', removed: [], added: [] }) - 1
          ] as {
            kind: 'change'
            removed: string[]
            added: string[]
          })
    if (line.op === 'remove') block.removed.push(line.text)
    else block.added.push(line.text)
  }
  return blocks
}
...
export function diffTokens(
  before: string,
  after: string
): { removed: TokenRun[]; added: TokenRun[] } {
  ...
  if (a.length > MAX_TOKENS || b.length > MAX_TOKENS) return whole()
  ...
  const removed: TokenRun[] = []
  const added: TokenRun[] = []
  // Coalesce adjacent tokens of the same kind so the DOM is runs, not one span per word.
  const push = (out: TokenRun[], text: string, changed: boolean) => {
...
*/
function bridgeGaps(runs: TokenRun[]): TokenRun[] {
...
export class TosiDiff extends Component<DiffParts> {
  ...
  static shadowStyleSpec = {
    ...
    */
    ...
    '.diff-add .text.changed': {
      background: varDefault.tosiDiffAddRunBg(
        'color-mix(in srgb, #22c55e 38%, transparent)'
      ),
    },
    '.diff-remove .text.changed': {
      background: varDefault.tosiDiffRemoveRunBg(
        'color-mix(in srgb, #ef4444 38%, transparent)'
      ),
    },
  ...
  every read of `value`.
  */
  private _resolutions: DiffResolution[] = []
  ...
  private setAll(choice: DiffResolution): void {
    this.resolutions = this.resolutions.map(() => choice)
    this.dispatchEvent(new Event('change'))
  }

  acceptAll = (): void => this.setAll('modified')
  rejectAll = (): void => this.setAll('original')

  private syncResolutions(): void {
    if (
      this.original === this.resolvedOriginal &&
  ...
  private choose = (event: Event): void => {
    ...
    */
    ...
    const choice = picked.dataset.choice as DiffResolution
  ...
  private lineElement(
    ...
    */
    const pickable =
      hunk !== undefined && (op === 'add' || op === 'remove')
        ? {
    ...
    */
    ...
    const body =
      runs !== undefined && runs.length > 0
        ? runs.map((run) =>
            span({ class: run.changed ? 'text changed' : 'text' }, run.text)
          )
    ...
    return div(
      ...
      span({ class: 'marker' }, MARKER[op]),
      span({ class: 'line-text' }, ...body)
    )
  }
    ...
    this.parts.body.replaceChildren(
      ...this.blocks.map((block) => {
        if (block.kind === 'context') {
          return div(
            {},
            ...block.lines.map((text) => this.lineElement('context', text))
          )
        }
        changeIndex += 1
        const index = changeIndex
        const choice = this.resolutions[index] ?? 'modified'
        const pick = (label: string, value: DiffResolution) =>
          button(
            {
              type: 'button',
        ...
        return div(
          { class: 'diff-hunk', onClick: this.choose },
          div(
            { class: 'diff-choices' },
            pick(this.originalLabel, 'original'),
            pick(this.modifiedLabel, 'modified')
          ),
          /*
          ...
          */
          ...block.removed.map((text, k) =>
            this.lineElement(
              'remove',
              text,
              choice === 'original',
              block.added[k] === undefined
                ? undefined
                : diffTokens(text, block.added[k]).removed,
              index
            )
          ),
          ...block.added.map((text, k) =>
            this.lineElement(
              'add',
              text,
              choice === 'modified',
              block.removed[k] === undefined
                ? undefined
                : diffTokens(block.removed[k], text).added,
              index
            )
          )
        )
...
export const tosiDiff = TosiDiff.elementCreator() as ElementCreator<TosiDiff>
