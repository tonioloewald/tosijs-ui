...
export class TosiDiff extends Component<DiffParts> {
  ...
  */
  ...
  private lineElement(op: DiffOp, text: string, chosen = true): HTMLElement {
    return div(
    ...
    this.parts.body.replaceChildren(
      ...this.blocks.map((block) => {
        ...
        return div(
          ...
          ...block.removed.map((text) =>
            this.lineElement('remove', text, choice === 'original')
          ),
          ...block.added.map((text) =>
            this.lineElement('add', text, choice === 'modified')
          )
        )
      })
    )
