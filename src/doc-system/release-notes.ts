...
export async function lastVersionTag(): Promise<string> {
  ...
  */
  const described = await $`git describe --tags --abbrev=0 --exclude=*-*`
    .nothrow()
    .quiet()
  if (described.exitCode === 0) return described.stdout.toString().trim()
...
export function renderSection(
  ...
  const issues = [...new Set(all.flatMap((b) => b.issues))].sort(
    (a, b) => a - b
  )
  if (issues.length) {
...
export function unsupportedClaims(records: CommitRecord[]): CommitRecord[] {
  return records.filter(
    (r) =>
      !DOCS_COMMIT.test(r.subject) &&
      r.bullets.some((b) => b.tag === 'fix' || b.tag === 'new') &&
      isDocsOnly(r.files)
  )
}

// ── does the version match what actually changed? (#78, #79) ─────────────────
...
export function classifyBump(from: string, to: string): BumpKind {
  ...
  if ([fM, fm, fp, tM, tm, tp].some((n) => Number.isNaN(n))) return 'unknown'
  if (to.includes('-')) return 'prerelease'
...
export function bumpConcerns(opts: {
  ...
  if (breaks.length) {
    out.push({
      level: 'block',
      reason:
        'a [break] annotation in a PATCH — the contract adopters rely on is that a patch never breaks them',
      evidence: breaks.map((b) => `${b.sha} ${b.text.slice(0, 90)}`),
    })
  }

  const sensitive = changedPaths.filter((p) =>
    SENSITIVE_PATHS.some((s) => p.includes(s))
  )
  if (sensitive.length) {
    out.push({
      level: 'warn',
  ...
  if (changes.length) {
    out.push({
      level: 'warn',
      reason:
        'a [change] annotation in a patch — fine when it is additive (a widened peer range), wrong when behaviour moved',
