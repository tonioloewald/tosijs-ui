...
exclusion does not.
*/
function normalizeDeclaration(value: string | string[]): string[] | null {
  const raw = (Array.isArray(value) ? value : [value])
    .map((v) => String(v).trim())
    .filter(Boolean)
  if (!raw.length) return null // `""` or `[]` — nothing said
  /*
  ...
  */
  if (raw.some((v) => v.toLowerCase() === NO_BOOK)) return []
  const out: string[] = []
  for (const v of raw) {
 ...
 */
export function resolveBooks<T extends BookTargetDoc>(
  doc: T,
  docs: T[],
  slugMap: SlugMap = {}
): string[] {
