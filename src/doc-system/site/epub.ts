...
function htmlToXhtml(
  ...
  if (transform) transform(win.document)
...
function injectExampleLinks(doc: any, pageUrl: string): void {
  ...
  for (let i = 0; i < pres.length; i += 1) {
    ...
    while (
      i < pres.length - 1 &&
      (pres[i] as any).nextElementSibling === pres[i + 1]
    ) {
      group.push(pres[i + 1])
      i += 1
    }
...
export async function buildEpub(
  ...
  */
  ...
  const visible: Doc[] = volumes.get(opts.bookTarget ?? DEFAULT_BOOK) ?? []
  /*
  ...
  */
  if (visible.length === 0) {
    ...
    throw new Error(
      `ePub volume ${JSON.stringify(
        opts.bookTarget ?? '(default)'
      )} has no documents.\n` +
  ...
  */
  ...
  const baseIdentifier =
    config.baseUrl || `urn:tosijs-book:${slugify(baseTitle)}`
  const meta: BookMeta = {
    title: volumeTitle,
  ...
  fs.writeFileSync(
    path.join(buildDir, 'OEBPS', 'contents.xhtml'),
    tocPageXhtml(roots, fileFor)
  )
  fs.writeFileSync(
