...
interface KnownBrowser {
  /** the macOS application name AppleScript targets */
  app: string
  flavor: Flavor
  /** substring of the app bundle as it appears in `ps` output (running-detection) */
  psMatch: string
  /** aliases a user may pass via config / $BROWSER (lowercased) */
  aliases: string[]
}
...
function findByAlias(name: string): KnownBrowser | undefined {
  const n = name.trim().toLowerCase()
  return KNOWN_BROWSERS.find(
    (b) => b.app.toLowerCase() === n || b.aliases.includes(n)
  )
}

function firstRunning(running: string[]): KnownBrowser | undefined {
...
export function appleScript(
  ...
  if (flavor === 'safari') {
    return [
      ...
      `    else`,
      `      tell window 1 to set current tab to (make new tab with properties {URL:${JSON.stringify(
        url
      )}})`,
      `    end if`,
      `  end if`,
  ...
  return [
    ...
    `    else`,
    `      tell front window to make new tab with properties {URL:${JSON.stringify(
      url
    )}}`,
    `    end if`,
    `  end if`,
...
export type OpenPlan =
  ...
  | { action: 'applescript'; app: string; flavor: Flavor; script: string }
  | { action: 'exec'; cmd: string[]; reuse: boolean }

export interface OpenPlanInput {
  url: string
  /** the `openBrowser` site-config value */
  setting: boolean | string | undefined
  env: { BROWSER?: string; CI?: string }
  platform: string
  isTTY: boolean
 ...
 */
export function buildOpenPlan(input: OpenPlanInput): OpenPlan {
  ...
  if (env.CI === 'true') return { action: 'skip', reason: 'CI' }
  if (!isTTY) return { action: 'skip', reason: 'not a TTY (non-interactive)' }
  ...
  const requestedName =
    browserEnv || (typeof setting === 'string' ? setting.trim() : '')
...
export function osascriptArgv(script: string): string[] {
  return ['osascript', ...script.split('\n').flatMap((line) => ['-e', line])]
}

async function detectRunningApps(): Promise<string[]> {
  try {
...
export async function openDevBrowser(opts: {
  ...
  try {
    const cmd =
      plan.action === 'applescript' ? osascriptArgv(plan.script) : plan.cmd
