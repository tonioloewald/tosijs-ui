...
// and cause a rebuild loop. The theme stylesheet is generated by ./generate-css.ts
 ...
 */
export function pageDepth(slug: string): number {
  return slug === '' ? 0 : 1
}
 ...
 */
export function relativeUrl(depth: number, p: string): string {
...
export async function generateSite(
  ...
  for (const doc of docs) {
    ...
    if (seen.has(slug)) {
      console.warn(
        `generate-site: slug "${slug}" used by both ${seen.get(slug)} and ${
          doc.filename
        }`
      )
    }
  ...
  if (config.localizedStrings) {
    const localizedPath = (
      config.localizedUrl || '/localized-strings.txt'
    ).replace(/^\//, '')
  ...
  if (config.baseUrl) {
    ...
    if (!robots.includes('Sitemap:')) {
      ...
      await Bun.write(
        robotsPath,
        `${base}Sitemap: ${config.baseUrl}/sitemap.xml\n`
      )
    }
  }
