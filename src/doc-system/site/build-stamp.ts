...
const defaultGit: GitReader = async (args) => {
  try {
 ...
 */
export async function gatherBuildStamp(
  opts: { generator: string; site?: string; git?: GitReader } = {
    generator: 'unknown',
  }
): Promise<BuildStamp> {
  const git = opts.git ?? defaultGit
  const stamp: BuildStamp = { generator: opts.generator }
  if (opts.site) stamp.site = opts.site
  ...
  if (commitTime) stamp.commitTime = commitTime

  return stamp
}

/** Serialize for `/version.json` (trailing newline so the file is diff-friendly). */
export function serializeBuildStamp(stamp: BuildStamp): string {
...
*/
export function stampToWrite(
  previousJson: string | null,
  fresh: BuildStamp,
  contentHash: string
): string {
  const next = serializeBuildStamp({ ...fresh, contentHash })
  if (!previousJson) return next
  let previous: BuildStamp
  try {
  ...
  if (!previous.contentHash) return next
  if (previous.contentHash !== contentHash) return next
  ...
  if (previous.generator !== fresh.generator) return next
  if (previous.site !== fresh.site) return next
  return previousJson
}
...
export async function hashOutput(
  ...
  readFile: (p: string) => Promise<Uint8Array | null>
): Promise<string> {
