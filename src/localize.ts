 ...
 */
export function applyLocalized(el: Element): void {
...
function* walkLocalizedRoots(root: Document | ShadowRoot): Generator<Element> {
  ...
  for (const el of Array.from(root.querySelectorAll('*'))) {
    const shadow = (el as Element & { shadowRoot?: ShadowRoot | null })
      .shadowRoot
    if (shadow) yield* walkLocalizedRoots(shadow)
  }
}

function applyLocalizedEverywhere(): void {
    ...
    localizedObserver = new MutationObserver((records) => {
      for (const rec of records) {
        ...
        for (const node of Array.from(rec.addedNodes)) {
          ...
          if (node.hasAttribute(LOCALIZED_ATTR)) applyLocalized(node)
          for (const el of Array.from(
            node.querySelectorAll(`[${LOCALIZED_ATTR}]`)
          )) {
            applyLocalized(el)
          }
        }
