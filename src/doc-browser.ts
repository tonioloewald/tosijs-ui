...
export function createDocBrowser(options: DocBrowserOptions): HTMLElement {
  ...
  */
  ...
  const applyFullScreen = () => {
    ...
    */
    ...
    business.
    */
    if (wantsFullScreen) sidenav.navVisible = false
    else sidenav.alwaysCompact = false
  ...
  */
  ...
  const loadSource = async (p: string): Promise<string | null> => {
    ...
    try {
      ...
      if (r.ok && !/text\/html/i.test(contentType)) return await r.text()
    } catch {
  ...
  */
  const testsMenuItem = (): any => {
    ...
    return {
      ...
      action: () =>
        testManager.enabled.value ? disableTests() : enableTests(),
    }
  }
  ...
  */
  ...
  const runBackgroundTests = async () => {
    ...
    */
    const vp = (globalThis as any).__tosiTestViewport as
  ...
  */
  ...
  ;(container as any).testsMenuItem = testsMenuItem
