...
export function warnNoValidator(what: string): void {
  if (current || warned.has('validate')) return
  warned.add('validate')
  console.warn(
 ...
 */
export function warnCannotInfer(): void {
  if (warned.has('infer')) return
  warned.add('infer')
  console.warn(
    `tosijs-ui: <tosi-schema-form> has no \`schema\` and no validator to infer one from, so ` +
