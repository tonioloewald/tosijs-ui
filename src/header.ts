...
export class TosiHeader extends Component<HeaderParts> {
  ...
  themePrefs: ThemePrefs | null = null
  menuItems: any[] = []
  ...
  content = () => {
    ...
    if (this.projectName) {
      parts.push(
        a(
          { part: 'title', class: 'title-link', href: '/' },
          h2(this.projectName)
        )
      )
    }
    ...
    for (const [key, url] of Object.entries(this.projectLinks)) {
      ...
      parts.push(
        a(
          { class: 'iconic', title: key, target: '_blank', href: url },
          linkIcons[key]()
        )
      )
    }
      ...
      button(
        {
          ...
          class: 'iconic',
          title: 'settings',
          onClick: (event: Event) => this.showSettingsMenu(event),
        },
        icons.moreVertical()
      )
    )
  ...
  showSettingsMenu(event: Event): void {
    ...
    if (this.showLocale && i18n.localeOptions.value.length > 0) {
      ...
      items.push({
        caption: 'Language',
        icon: 'globe',
        menuItems: i18n.localeOptions.value.map(
          (locale: { caption: string; icon: string; value: string }) => ({
      ...
      const themeItems: any[] = [
        {
          ...
          action() {
            if (prefs) prefs.theme.value = 'system'
          },
        },
        {
          caption: 'Dark',
          ...
          action() {
            if (prefs) prefs.theme.value = 'dark'
          },
        },
        {
          caption: 'Light',
          ...
          action() {
            if (prefs) prefs.theme.value = 'light'
          },
        },
        null,
        {
          ...
          action() {
            if (prefs) prefs.highContrast.value = !prefs.highContrast.value
          },
        },
      ]
