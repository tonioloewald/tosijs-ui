...
export function contextVarName(key: string): string {
  ...
  */
  if (!stripped) return '_ctx'
  if (/^[0-9]/.test(stripped) || RESERVED.has(stripped)) return `_${stripped}`
  return stripped
}
 ...
 * replaces.
 */
export function contextParamNames(keys: string[]): string[] {
  ...
  return keys.map((key) => {
    ...
    if (already !== undefined) {
      throw new Error(
        `example context keys ${JSON.stringify(already)} and ${JSON.stringify(
          key
        )} both reduce to the identifier "${name}", so an import from one of them could not be ` +
...
type FromTsFn = (
  ...
  options?: {
    emitTJS?: boolean
    /**
     * Which TypeScript build to lazy-load. Declared here because this local type
     * mirrors the BROWSER entry (`BrowserFromTSOptions`), not the node
     * `FromTSOptions` — only the browser one carries it, and omitting it silently
     * falls back to tjs-lang's unpinned `esm.sh/typescript@5`.
     */
