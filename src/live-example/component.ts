...
import { Dialect, ExampleContext, ExampleParts, TransformFn } from './types.js'
import {
  loadTransform,
  loadTjsTestApi,
  rewriteImports,
 ...
 */
export function pageTestCount(): number {
  if (typeof document === 'undefined') return 0
  return Array.from(document.querySelectorAll('tosi-example')).filter(
    (el) => !!(el as LiveExample).test
  ).length
}
...
export class LiveExample extends Component<ExampleParts> {
   ...
   * content.
   */
  private get isTestOnly(): boolean {
    const blank = (v: string | undefined) => !v || v.trim() === ''
    return (
      !blank(this.test) && blank(this.js) && blank(this.html) && blank(this.css)
    )
  }
  ...
  handleTestsToggle = (event: Event): void => {
    if ((event.target as HTMLInputElement).checked) enableTests()
    else disableTests()
  }
    ...
    div(
      ...
      */
      tosiPocketBar(
        ...
        button(
          { title: 'view/edit code', onClick: this.showCode },
          icons.edit2()
        ),
        button(
          { title: 'toggle preview size', onClick: this.toggleMaximize },
  ...
  */
  collapseWidgetsAfterAction = (event: Event): void => {
    ...
    const bar = this.parts.exampleWidgets as HTMLElement & {
      close?: () => void
    }
    const acted = event
      .composedPath()
  ...
  private displayTestResults(): void {
    ...
    if (!results || results.tests.length === 0) {
      ...
      this.classList.remove('-test-passed', '-test-failed')
      if (exampleWidgets) exampleWidgets.title = 'no tests'
      return
    }
