...
export class TosiTable extends WebComponent {
  ...
  */
  private stampSelection(rows: any[], select: boolean): void {
    for (const row of rows) {
  ...
  */
  private applySelection(rows: any[], select: boolean): void {
    /*
    ...
    */
    ...
    if (select && !this.multiple && rows.length > 1) {
      ...
      */
      console.warn(
        `<tosi-table>.selectRows: ${rows.length} rows given but multiple=false — ` +
          `keeping the last. Set \`multiple\` to select more than one.`
      )
      rows = rows.slice(-1)
    }
    // Single-select REPLACES, exactly as a plain click does.
    if (select && !this.multiple) this.stampSelection(this.array, false)
