...
export class TosiCrud extends WebComponent {
    ...
    */
    ...
    async whenIdle() {
        if (this._pending > 0) {
            await new Promise((resolve) => this._idle.push(resolve));
        }
    ...
    */
    async run(operation, work) {
        ...
        try {
            ...
            this.dispatchEvent(new CustomEvent('error', { detail: { operation, error } }));
            throw error;
        }
        finally {
            this._pending--;
    ...
    async refresh() {
        ...
        const rows = await this.run('list', () => this._store.list({ search: this.search || undefined }));
    ...
    select(record) {
        ...
        this._hash?.set('id', id === undefined || id === null ? undefined : String(id), { push: true });
        this.queueRender();
    ...
    async save() {
        if (!this._store?.save || !this._selected)
            return;
    ...
    async remove() {
        if (!this._store?.delete || !this._selected)
            return;
...
export const tosiCrud = TosiCrud.elementCreator();
