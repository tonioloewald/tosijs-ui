...
export class TosiTable extends WebComponent {
    ...
    */
    _groupIdMemo = new WeakMap();
    get groupIdFn() {
        const groupId = resolveRowGroupId(this._rowGroupId, this._nonRepeatingGroupedRowCells);
        if (!groupId)
            return null;
    ...
    */
    ...
    fieldFor(prop) {
        ...
        if (!byProp.has(prop))
            byProp.set(prop, fieldForProperty(schema, prop));
    ...
    */
    ...
    validateCell(item, prop, _value) {
        ...
        const errors = collectErrors((onError) => validator.validate(tosiValue(item), this.schema, {
            onError,
    ...
    */
    buildEditableCell(col, colIndex, si, style, repeats) {
        ...
        */
        ...
        let cell;
        if (field?.kind === 'enum') {
            cell = select({ ...shared, bind: displayBinding }, ...(field.required ? [] : [option({ value: '' }, '—')]), ...(field.options ?? []).map((o) => option({ value: String(o.value) }, o.label)));
        }
        else {
    ...
    */
    ...
    buildHeader(cols, stickyInfo) {
        const headerCells = cols.map((col, i) => this.buildHeaderCell(col, i, stickyInfo[i]));
        return div({ class: 'thead', role: 'rowgroup' }, div({ class: 'tr', role: 'row' }, ...headerCells));
    }
    ...
    */
    captureScrollAnchor() {
        ...
        */
        ...
        const previous = tosiValue(this.rowData.visible) ?? [];
        for (const el of area.querySelectorAll('.tr')) {
            if (el.closest('.thead') || el.classList.contains('row-pinned'))
                continue;
            const rect = el.getBoundingClientRect();
            if (rect.height === 0 || rect.bottom <= areaTop)
                continue;
        ...
        */
        ...
        const step = () => {
            ...
            if (--framesLeft > 0)
                ...
                restore.abort(); // budget spent — drop the gesture listeners
        };
        /*
    ...
    render() {
        ...
        */
        if (this.rowHeight === 0 && baseData.length > NON_VIRTUAL_ROW_ADVICE) {
            console.warn(`🚨 <tosi-table>: ${baseData.length.toLocaleString()} rows with rowHeight 0 — NOT VIRTUAL.\n` +
