 ...
 */
export function tokenRunsForLines(lines) {
    /*
    ...
    */
    ...
    while (i < lines.length) {
        ...
        let j = i;
        while (j < lines.length && lines[j].op !== 'context')
            j += 1;
        const removes = [];
        const adds = [];
        for (let k = i; k < j; k += 1) {
        ...
        */
        if (!this.resolvable) {
            ...
            */
            ...
            this.parts.body.replaceChildren(...lines.map((line, k) => this.lineElement(line.op, line.text, true, runs[k])));
            return;
        }
        let changeIndex = -1;
