 ...
 */
export function grammarFor(fenceLang) {
...
export function loadableGrammars() {
...
export function langOfClass(className) {
    return (String(className ?? '').match(LANGUAGE_CLASS)?.[1] ?? '').toLowerCase();
}
let prism;
...
export async function ensureGrammar(lang) {
        ...
        */
        ...
        try {
            ...
            */
            if (!warnedNoPrism) {
                warnedNoPrism = true;
                console.warn('prismjs could not be loaded — static code blocks will not be syntax-highlighted ' +
    ...
    */
    ...
    if (prism.languages[grammar])
        return true;
    if (BUILTIN.has(grammar))
        ...
        load = (async () => {
            ...
            */
            ...
            try {
                ...
                console.warn(`prismjs grammar "${grammar}" failed to load — code blocks in that language ` +
                    `will not be highlighted.`, err);
                return false;
            }
            return Boolean(prism.languages[grammar]);
        })();
...
const isPlain = (code) => !code.querySelector('.token') && !code.hasAttribute('data-highlighted');
/**
...
export async function highlightBlocks(root, opts = {}) {
    ...
    */
    ...
    if (blocks.length === 0)
        return 0;
    const langOf = (code) => langOfClass(code.className);
 ...
 */
export async function highlightHtml(html, policy = 'auto') {
    ...
    */
    return html.replace(/(<pre([^>]*)>)<code class="language-([A-Za-z0-9_+#-]+)">([\s\S]*?)<\/code>/g, (whole, preTag, preAttrs, lang, body) => {
        ...
        if (isLiveFence(l, mode, policy))
            return whole;
        if (!available.has(l))
            return whole;
        if (body.includes('class="token'))
            return whole;
