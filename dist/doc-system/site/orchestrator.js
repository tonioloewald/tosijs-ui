...
import { bundleBuildArgs, bundleDeclarationArgs, extensionlessImports, } from './library-bundle.js';
...
export function shouldCleanDist(config) {
    ...
    if (config.libraryBuild)
        return false;
    return (config.emitLibrary === true ||
          ...
          */
            ...
            if (config.libraryBuild) {
                ...
                for (const [what, argv] of [
                    ...
                    if ((await child.exited) !== 0) {
                        ...
                        console.error(`❌ libraryBundle: ${what} FAILED — dist/ is incomplete. Build marked failed.` +
                ...
                if (!libraryBuildFailed) {
                    ...
                    if (bad.length) {
                        console.error(`❌ libraryBundle: dist/ has relative imports without a file extension, which Node cannot load:\n  ${bad.join('\n  ')}`);
                        libraryBuildFailed = true;
                    }
                }
            }
            ...
            */
            if (!libraryBuildFailed &&
                ...
                const bad = extensionlessImports(DIST);
                if (bad.length) {
                    console.warn(`⚠️  dist/ has ${bad.length} relative import(s) without a file extension — Node rejects them ` +
                        `(ERR_MODULE_NOT_FOUND), even though Bun and bundlers resolve them. Use \`libraryBundle\` ` +
                        `instead of ${config.libraryTsconfig ? 'libraryTsconfig' : 'emitLibrary'} (tosijs-ui#169). First: ${bad[0]}`);
                }
            }
            ...
            if (config.bundleEntry) {
              ...
              */
                const HYDRATE_DIR = path.resolve(PROJECT_ROOT, 'node_modules/.cache', `tosijs-hydrate-${process.pid}`);
