...
export async function buildSite(config) {
    ...
    console.log(`dist/iife.js: ${(iifeFile.byteLength / 1024).toFixed(1)}kb (${(iifeGzip.length / 1024).toFixed(1)}kb gzip)`);
    ...
    await $ `bun ./src/doc-system/site/generate-css.ts ${PUBLIC}/doc-system.css ${JSON.stringify(config.theme || {})}`.text();
