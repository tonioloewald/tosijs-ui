...
export function lockPathFor(root, dir = tmpdir()) {
 ...
 */
export function lockDecision(existing, self, isAlive) {
    if (!existing)
        return { action: 'take' };
...
export function describeHolder(holder) {
    if (!holder)
        return 'Nothing is building here — the lock is free.\n';
    const what = holder.role === 'dev-server'
        ? `A dev server (pid ${holder.pid}${holder.port ? `, port ${holder.port}` : ''})`
 ...
 */
export function acquireBuildLock(root, role, opts = {}) {
    const file = lockPathFor(root, opts.dir);
    const pid = opts.pid ?? process.pid;
    const decision = lockDecision(readLock(file), { pid }, opts.isAlive ?? isProcessAlive);
