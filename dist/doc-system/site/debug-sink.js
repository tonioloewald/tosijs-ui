 ...
 */
export function sinkPathFor(setting, root, dir = tmpdir()) {
    if (!setting)
        return null;
 ...
 */
export function appendToSink(file, body, opts = {}) {
    ...
    const sizeOf = opts.sizeOf ?? ((p) => (existsSync(p) ? statSync(p).size : 0));
