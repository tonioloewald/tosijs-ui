...
export function relocateSources(sources, fromDir, toDir) {
    return sources.map((source) => /^[a-z][a-z0-9+.-]*:/i.test(source)
        ? source
        : path
 ...
 */
export function relocateSourcemaps(builtDir, servedDir) {
    for (const map of mapsUnder(builtDir)) {
        ...
        if (Array.isArray(json.sources)) {
            json.sources = relocateSources(json.sources, path.dirname(map), path.dirname(target));
        }
