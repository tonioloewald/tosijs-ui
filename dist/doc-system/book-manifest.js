...
function docMatchesKey(doc, key) {
    const k = key.trim().toLowerCase();
    return (doc.filename.toLowerCase() === k ||
 ...
 */
export function selectBookDocs(docs, manifest) {
    if (!manifest)
        return docs.slice();
    // 1) select ────────────────────────────────────────────────────────────────
