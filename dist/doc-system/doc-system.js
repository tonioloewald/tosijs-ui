...
export async function fetchCorpus(url) {
    let lastError;
    for (let attempt = 0; attempt < CORPUS_ATTEMPTS; attempt++) {
        if (attempt > 0) {
            // Full jitter: a random point in [0, backoff), not backoff itself.
            const ceiling = Math.min(CORPUS_MAX_DELAY_MS, CORPUS_BASE_DELAY_MS * 2 ** (attempt - 1));
            ...
            if (!response.ok) {
                ...
                */
                ...
                if (Number.isFinite(after) && after > 0) {
                    await new Promise((resolve) => setTimeout(resolve, Math.min(after * 1000, CORPUS_MAX_DELAY_MS)));
                }
                continue;
            }
...
export class TosiDocSystem extends Component {
    ...
    applyStyles() {
        ...
        if (!document.querySelector('link[data-tosi-doc-system]')) {
            StyleSheet('tosi-doc-system', docSystemStyleSpec({
    ...
    settingsButton() {
        return button({
            ...
            onClick: (event) => {
                ...
                */
                const sourceItems = this.browser?.sourceMenuItems?.() ?? [];
                ...
                returns null.
                */
                const testsItem = this.browser?.testsMenuItem?.() ?? null;
                if (testsItem) {
