...
// --- data-tosi-localized directive --------------------------------------
const LOCALIZED_ATTR = 'data-tosi-localized';
/**
 ...
 */
export function applyLocalized(el) {
...
// Module-level reference keeps the observer (and its callback) alive. happy-dom
...
if (typeof document !== 'undefined' &&
    ...
    const startObserver = () => {
        ...
        // Pick up any elements that were already in the DOM before this module ran.
        applyLocalizedEverywhere();
    };
    startObserver();
}
...
export function initLocalization(localizedStrings) {
    ...
    if (locales && languages && emoji && strings) {
        ...
        i18n.stringMap.value = strings.reduce((map, strings) => {
        ...
        if (!i18n.locales.value.includes(i18n.locale.value)) {
            ...
            i18n.locale.value =
                i18n.locales.value.find((locale) => locale.substring(0, 2) === language) || i18n.locales.value[0];
        }
        updateLocalized();
    }
}
...
function interpolate(text, values) {
    return text.replace(/\{([A-Za-z0-9_]+)\}/g, (whole, name) => Object.prototype.hasOwnProperty.call(values, name)
        ? String(values[name])
        : whole);
}
export function localize(ref, values) {
    if (values)
        return interpolate(localize(ref), values);
    if (ref.endsWith('…')) {
 ...
 */
export function localizePhrase(key, fragments) {
    /*
    ...
    */
    const index = i18n.locales.value.indexOf(i18n.locale.value);
    if (index > -1) {
        const map = tosiValue(i18n.stringMap);
