...
export class TosiPocketBar extends WebComponent {
    ...
    get floatPosition() {
        return (this.direction === 'auto' ? 'side' : this.direction);
    }
    reposition = () => {
        if (!this.hydrated)
            return;
        positionFloat(this.parts.bar, this.parts.handle, this.floatPosition, 'remain', 'remain');
    };
    ...
    static shadowStyleSpec = {
        ...
        ':host [part="handle"]': {
            ...
            background: varDefault.tosiPocketBg('color-mix(in srgb, var(--tosi-bg, #fff) 85%, transparent)'),
        ...
        ':host [part="bar"]': {
            ...
            background: varDefault.tosiPocketBg('color-mix(in srgb, var(--tosi-bg, #fff) 82%, transparent)'),
            backdropFilter: 'blur(12px)',
            borderRadius: vars.spacing,
...
export const tosiPocketBar = TosiPocketBar.elementCreator();
