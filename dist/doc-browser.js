...
export function createDocBrowser(options) {
    ...
    const headerContent = [
        button({
            ...
            onClick() {
                if (sidenav)
                    sidenav.contentVisible = !sidenav.contentVisible;
            },
        }, icons.menu()),
    ...
    */
    ...
    const container = div({
        ...
        onClick(event) {
            ...
            const btn = event.target.closest('.view-source');
            if (btn)
                openSourceMenu(btn);
        },
        bind: {
    ...
    }, icons.code({ style: { _xinIconSize: 16 } }), 'Source'), docContent)));
    ...
    */
    if (isTestFrame && testFrameFilename) {
        ...
        const signalDone = () => {
            const stalled = unsettledExamples([...container.querySelectorAll('tosi-example')].map((ex) => ({
                element: ex,
                test: ex.test,
