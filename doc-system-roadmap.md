...
- **CSS-variable namespace unification** — the doc-system is themed in a _legacy_
...
-text/-bg-inset` onto the legacy vars _as references_, so components follow the theme
  (and flip in `.darkmode`) automatically. **End-state:** theme the doc-system _in_
  `--tosi-*` directly — retire the legacy family — and namespace doc-system-_specific_
...
- **Dynamic theme system — retire the `.darkmode` class for a bound stylesheet.** _(Direction,
  ...
  - **A drop-in customization widget** — a consumer injects the theme editor into _their_ dev
