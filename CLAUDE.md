...
- **Unit tests** (`src/*.test.ts`): Run with `bun test`. Use `happy-dom` for DOM simulation (preloaded via `bunfig.toml` → `src/test-setup.ts`, which is also published as `tosijs-ui/test-setup` for adopters, #170). Import from `bun:test`.
